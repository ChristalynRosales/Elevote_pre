              ...
              <form class="form-horizontal" method="POST" action="candidates_edit.php">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9">
                      ...
                      // Get the current candidate's position ID 
                        $sqlCandidate = "SELECT position_id FROM candidates WHERE id = :candidateId";
                          $stmtCandidate = $conn->prepare($sqlCandidate);
                          $stmtCandidate->bindParam(':candidateId', $candidateId, PDO::PARAM_INT);
                        $stmtCandidate->execute();
                      $candidatePositionId = $stmtCandidate->fetchColumn();

                    foreach ($positions as $row) {
                      $selected = ($row['id'] == $candidatePositionId) ? 'selected' : '';
